...
from reader import InventoryReader, JamecoInventoryReader, TaydaInventoryReader
...
def detect_supplier_from_file(file: Union[str, Path]) -> Optional[SUPPLIER]:
    PARSERS = {
        '.pdf': detect_supplier_from_pdf,
        '.eml': detect_supplier_from_email
    }
    ...
    if file_type not in [s.value.file_type for s in SUPPLIER]:
        raise ValueError(
            f"Cannot parse from file of type {file_type}"
        )
...
def get_reader_from_file(file: Union[str, Path]) -> InventoryReader:
    ...
    if supplier is None:
        raise ValueError(
            f"Unable to detect supplier from {file}."
        )
